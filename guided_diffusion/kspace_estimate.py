...
class TranslationTransform2D():
    ...
    def __call__(self, k_space, phase_shift):
...
#             x_shift * torch.linspace(0, 1, x_shape)[None, :, None].to(device) +
#             y_shift * torch.linspace(0, 1, y_shape)[None, None, :].to(device))
#         print(f" kspace shape {k_space.shape} ps {phase_shift.shape}")
...
#     print(f"gridx {grid_x.shape}  gridy {grid_y.shape} lenx {lenx}")
...
class RandomMotionTransformKnown():

    def __init__(self):
        super(RandomMotionTransformKnown, self).__init__()
#         self.xy_max = xy_max
#         self.theta_max = theta_max
#         self.noise_lvl = noise_lvl
        self.T = RandomTranslationTransform()
#         self.R = RandomRotationTransform()

    def __call__(self, k_space,phase_shift,rot_vector):
        
        input_image = abs(IFt(k_space))
        rotated_image = rotate_image(input_image,rot_vector)
        k_space_rotated = Ft(rotated_image)
        k_space_rotated[:,114:142,:]=k_space[:,114:142,:]
        
        k_space = self.T(k_space_rotated,phase_shift)
