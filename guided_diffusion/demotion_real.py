...
#     print(f"xf {xf} yf {yf} xc {xc} yc {yc}")
    img_rot[:, :] = (
        (1 - a) * (1 - b) * img[xf, yf] +
        (1 - a) * b * img[xf, yc] +
        a * (1 - b) * img[xc, yf] +
...
#         print(f"ks {ks.angle().shape} phase_shift {phase_shift.shape}")
        
        
        yp_ks = ks.abs().cuda() * (1j * (ks.angle().cuda() + phase_shift)).exp()
        
#         print(yp_ks.shape)
#         plt.imshow(np.log(abs(yp_ks).cpu().detach().numpy()),cmap='gray')
#         plt.show()
...
#         plt.show()
        ...
        # Rotation
#         yp_img = img
#         print(f"\n img shape {img.shape}, rot vec {rot_vector.shape} \n")
...
#         plt.imshow(yp_img[0,0].cpu().detach().numpy(),cmap='gray')
#         plt.show()
        
        loss_net = (yp_img * 1e4).mean()
...
#     y_shifts = y_shifts * zero_middle
    ...
    yp_ks = ks.abs().cuda() * (1j * (ks.angle().cuda() + phase_shift)).exp()
    img = IFt(yp_ks).abs()
#     print(f"outside loop img {img.shape} rot {rot_vector.shape}")
    # Rotation
    yp_img = rotate_image(img,rot_vector).cuda()
