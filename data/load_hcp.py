...
class H5CachedDataset(Dataset):
    def __init__(self, datapath,
        ...
        self.test_datalist = niilist[:50]      #### testing on 50 volumes
        if mode=='train':
...
#             print(f" data shape {data['image'].shape} mask3d['image'] {mask3d['image'].shape}")
            res = {}
            res['image'] = data['image'] #*mask3d['image'][:,:,:,slicenum]
            res['filenum'] = filenum
            res['slicenum'] = slicenum
            res['idx'] = index
            return res['image'] ,mask3d['image'][:,:,:,slicenum], {"y":np.array((1))}
